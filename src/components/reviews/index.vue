    ...
    <section class="reviews">
        <div class="reviews__heading">
            ...
            <div class="reviews__rating">
                <span>{{ amountOfReviews }} отзывов</span>
                <StarRating :rating="totalRating" />
            </div>
        </div>
        <ReviewsItem v-for="review in currentReviews" :key="review.author" :review='review' />
        <button @click="togglelReviews" class="reviews__show-more"> {{ buttonText }} </button>
    </section>
</template>
...
export default {
    ...
    computed: {
        ...
        buttonText() {
            return this.reviewsLimit === this.reviews.length ? 'Свернуть' : 'Читать еще...'
        }
    },
    methods: {
        togglelReviews() {
